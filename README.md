...
This repository is the skeleton for the Andruha Messenger Object Storage Service. It contains package boundaries and operational HTTP infrastructure only. No messenger business behavior is implemented.
...
uv run pytest
docker build --target runtime --tag andruha/object-storage-service:local .
```
...
verified image to GHCR only for a version tag. Business APIs, the metadata schema, and
